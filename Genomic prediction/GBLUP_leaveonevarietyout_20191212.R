...
#Specify testing group
d6$Variety <- strtrim(d6$Clovershort,5)
grouptoleaveout=args[3]
testing=which(d6$Variety==grouptoleaveout)
print(testing)
datapointsusedfortesting=length(testing)
percentage=datapointsusedfortesting/nrow(d6)
Testingpopstat=cbind(datapointsusedfortesting,percentage)
colnames(Testingpopstat)=c("Number of observations used for testing","percentage of all observations")
write.table(Testingpopstat,paste("Testingpopstat",round,grouptoleaveout,".txt",sep=""),sep="\t")
...
write.table(h2,paste("h2",round,grouptoleaveout,".txt",sep=""),sep="\t")


# Prediction
print("Starting GBLUP prediction")
results=GP_GBLUP(testing)

All1=aggregate(as.numeric(results[, 2]), list(results[,1]), mean)
All4=aggregate(as.numeric(results[, 5]), list(results[,1]), mean)
All2=aggregate(as.numeric(results[, 3]), list(results[,1]), mean)
All3=aggregate(as.numeric(results[, 3]), list(results[,1]), mean)
...
filename=paste("Correlation_GBLUP_GPD",round,grouptoleaveout,".txt",sep="")
write.table(correlation,filename,sep="\t")

filename1=paste("Predictions_GBLUP_GPD",round,grouptoleaveout,".txt",sep="")
