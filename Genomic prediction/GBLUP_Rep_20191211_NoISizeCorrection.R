...
Correctedforallfixed <- lmer(growth_per_day ~ factor(roundRep) + factor(NS) + factor(EW) + factor(Rhizobium) + (1|Clover), data=d6) 
summary(Correctedforallfixed)

ycorr <- d6$growth_per_day - model.matrix( ~ factor(roundRep) + factor(NS) + factor(EW)  + factor(Rhizobium), data=d6) %*% fixef(Correctedforallfixed)
